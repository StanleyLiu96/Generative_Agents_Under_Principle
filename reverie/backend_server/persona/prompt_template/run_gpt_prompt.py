...
def run_gpt_prompt_affective_emotions(persona, event_description, test_input=None, verbose=False):
  """
  Ask GPT to tag an event with up to 5 emotions (from the approved vocab)
  and 0-100 integer scores, returned as a strict Python dict literal.
  """
  EMOTION_VOCAB = [
  ...
  def __func_clean_up(gpt_response, prompt=""):
    # Strict parsing to a dict with allowed emotions and int scores 0-100.
    cleaned = ast.literal_eval(gpt_response.strip())
    if not isinstance(cleaned, dict):
      raise ValueError("Response is not a dict.")
    filtered = {}
    for emo, score in cleaned.items():
      if emo not in EMOTION_VOCAB:
        continue
      val = int(score)
  ...
  def get_fail_safe():
    # Fallback to a neutral single emotion to avoid blocking perception.
    return {"interest": 0}

  gpt_param = {"model": "gpt-3.5-turbo-instruct", "max_tokens": 120,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/affective_emotion_v1.txt"
...
def run_gpt_prompt_identify_opposite_emotions(emotion_list, test_input=None, verbose=False):
  """
  Surface absolute opposite emotion pairs present in the provided list.
  This is used to disambiguate conflicting emotional signals during memory
  merges (e.g., joy vs. anger for a similar event).
  """
  def create_prompt_input(emotion_list, test_input=None):
    if test_input: 
      return test_input
    pretty = "\n".join(emotion_list)
    return [pretty]
  ...
  def __func_clean_up(gpt_response, prompt=""):
    cleaned_pairs = []
    for raw_line in gpt_response.split("\n"):
      raw_line = raw_line.strip().strip("-").strip()
      if not raw_line:
        continue
      if "|" in raw_line:
        left, right = raw_line.split("|", 1)
        cleaned_pairs.append((left.strip(), right.strip()))
      elif "," in raw_line:
        parts = [p.strip() for p in raw_line.split(",") if p.strip()]
        if len(parts) == 2:
          cleaned_pairs.append((parts[0], parts[1]))
    return cleaned_pairs

  def __func_validate(gpt_response, prompt=""): 
    try: 
      out = __func_clean_up(gpt_response, prompt)
  ...
  gpt_param = {"model": "gpt-3.5-turbo-instruct", "max_tokens": 120, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/identify_opposite_emotions_v1.txt"
  prompt_input = create_prompt_input(emotion_list, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_daily_plan(persona, 
  ...
  def create_prompt_input(persona, wake_up_hour, test_input=None):
    if test_input: return test_input
    prompt_input = []
    aff_context = persona.get_affective_prompt_context()
    prompt_input += [f"{persona.scratch.get_str_iss()}\n\nEmotionally salient memories:\n{aff_context}"]
...
def run_gpt_prompt_generate_next_convo_line(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input=None, verbose=False): 
  def create_prompt_input(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input=None): 
    # Verbose: fold the affective context directly into the retrieved summary
    # so downstream prompts see emotionally salient memories without changing
    # template arity.
    aff_context = persona.get_affective_prompt_context()
    retrieved_summary = f"{retrieved_summary}\n\nAffective memory highlights:\n{aff_context}"
    prompt_input = [persona.scratch.name, 
                    persona.scratch.get_str_iss(),
                    persona.scratch.name, 
                    interlocutor_desc, 
...
def run_gpt_generate_iterative_chat_utt(maze, init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None, verbose=False): 
  def create_prompt_input(maze, init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None):
    ...
    # Inject affective memory context so the LLM can lean on emotional state.
    aff_context = persona.get_affective_prompt_context()
    if aff_context:
      retrieved_str += f"\nAffective memory highlights:\n{aff_context}\n"
